...
export default function PatientDetail() {
  ...
  const lancerSepSain = async (irmId) => {
    ...
    try {
      ...
      setSepSainLoading(false)
    }
  }

  const lancerPrediction = async (irm) => {
    setPredLoading(true)
    setPredErreur(null)
    setPredIrmId(irm.id)
    try {
      const res = await fetch(`/api/predictions/prediction/${irm.id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      })
      ...
      setPredErreur('Erreur de connexion')
    } finally {
      setPredLoading(false)
    }
  }

  const lancerLSTM = async (irm) => {
    setLstmLoading(true)
    setLstmErreur(null)
    setLstmIrmId(irm.id)
    try {
      const res = await fetch(`/api/predictions/temporal/${irm.id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      })
      ...
      setLstmErreur('Erreur de connexion')
    } finally {
            ...
            <div className="glass-card" style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                {[
                  ...
                  { label: 'Score risque', value: '—', gradient: 'stat-gradient-violet', color: '#7c3aed' },
              ...
              {visites.length === 0 ? (
                ...
                <div key={v.id} className="glass-card" style={{ padding: '18px 22px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: v.notes || (v.tests_fonctionnels && Object.keys(v.tests_fonctionnels).length > 0) ? '12px' : '0' }}>
                    ...
                    {v.edss_score !== null && (
                      <span className={`badge ${v.edss_score <= 3 ? 'badge-emerald' : v.edss_score <= 6 ? 'badge-amber' : 'badge-red'}`}>
                        EDSS : {v.edss_score}
                      </span>
                    )}
                  </div>
                    ...
                    <div style={{ display: 'flex', gap: '6px', marginBottom: '8px', flexWrap: 'wrap' }}>
                      {Object.entries(v.tests_fonctionnels).map(([k, val]) => (
                        <span key={k} style={{
                          ...
                          borderRadius: '6px', background: '#f8f9fc', color: '#5a6070', textTransform: 'capitalize',
                        }}>
                          {k}: {val}
                        </span>
                      ))}
                ...
                <form onSubmit={handleUploadIRM}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                    ...
                    <div>
                      ...
                      <select value={sequenceType} onChange={e => setSequenceType(e.target.value)} className="select-light">
                        <option>FLAIR</option>
                        <option>T1</option>
                        <option>T2</option>
                        <option>Gd+</option>
                    ...
                    <div className="animate-slideUp" style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                      <div style={{
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                          {[
                            { val: predResultat.volume_lesions_futures, label: 'Voxels lésions', color: '#d97706' },
                            { val: `${predResultat.proba_rechute}%`, label: 'Probabilité', color: '#d97706' },
                            { val: '0.82', label: 'Dice', color: '#059669' },
                          ].map(({ val, label, color }) => (
                            <div key={label} style={{ textAlign: 'center', padding: '10px', background: 'white', borderRadius: '8px' }}>
                              <p style={{ fontSize: '18px', fontWeight: 700, color, margin: 0 }}>{val}</p>
                              <p style={{ fontSize: '11px', color: '#9ca3b0', margin: '2px 0 0' }}>{label}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  {lstmResultat && (
                    <div className="animate-slideUp" style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                      <div style={{
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                          {[
                            { val: lstmResultat.volume_lesions_futures, label: 'Voxels T4', color: '#7c3aed' },
                            { val: lstmResultat.n_irm_utilisees, label: 'IRM utilisées', color: '#7c3aed' },
                            { val: '0.74', label: 'Dice', color: '#059669' },
                          ].map(({ val, label, color }) => (
                            <div key={label} style={{ textAlign: 'center', padding: '10px', background: 'white', borderRadius: '8px' }}>
                              <p style={{ fontSize: '18px', fontWeight: 700, color, margin: 0 }}>{val}</p>
                              <p style={{ fontSize: '11px', color: '#9ca3b0', margin: '2px 0 0' }}>{label}</p>
